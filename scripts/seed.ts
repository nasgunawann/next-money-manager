...
const categories = {
  income: [
    { name: "Gaji", icon: "banknote", color: "#22c55e" },
    { name: "Freelance", icon: "briefcase", color: "#10b981" },
    { name: "Investasi", icon: "trending-up", color: "#14b8a6" },
  ],
  expense: [
    ...
    { name: "Hiburan", icon: "film", color: "#06b6d4" },
    { name: "Kesehatan", icon: "heart-pulse", color: "#84cc16" },
  ],
};
  ...
  try {
    ...
    for (let monthOffset = 0; monthOffset < 12; monthOffset++) {
      ...
      for (let i = 0; i < incomeCount; i++) {
        ...
        const category = createdCategories.filter(
          (c: any) => c.type === "income"
        ...
        transactions.push({
          ...
          description: `${category.name} - ${month.toLocaleDateString("id-ID", {
            month: "long",
            year: "numeric",
          })}`,
        });
      ...
      for (let i = 0; i < expenseCount; i++) {
        ...
        transactions.push({
          ...
          account_id: account.id,
          date: date.toISOString(),
          description: `${category.name} - ${date.toLocaleDateString("id-ID", {
            day: "numeric",
            month: "short",
          })}`,
        });
    ...
    for (let i = 0; i < transactions.length; i += batchSize) {
      ...
      console.log(
        `✅ Inserted ${Math.min(i + batchSize, transactions.length)}/${
          transactions.length
        } transactions`
      );
    }
